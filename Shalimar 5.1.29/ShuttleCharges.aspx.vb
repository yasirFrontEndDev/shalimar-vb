...
#Region " Form Events "
    ...
    Protected Sub Page_Load(ByVal sender As Object, ByVal e As System.EventArgs) Handles Me.Load
        ...
        objShuttle = New clsShuttle(objConnection)
...
#Region " Control Events "
    ...
    Private Sub grdDetails_InitializeLayout(ByVal sender As Object, ByVal e As Infragistics.WebUI.UltraWebGrid.LayoutEventArgs) Handles grdDetails.InitializeLayout
        ...
        With e.Layout.Grid
            ...
            .Columns.FromKey("Shuttle_Id").Hidden = True
            ...
            .Columns.FromKey("Fare_Amount").Header.Caption = "Shuttle Fare"
                            ...
                            If (row.Cells(i).Column.Key <> pk) Then
                                ...
                                Else
                                    ...
                                    ElseIf UCase(row.Cells(i).Column.Key) = "TERMINAL_ID" Then
                                        addedRow(i) = cboTerminals.SelectedValue
                                ...
                                If (row.Cells(i).Column.ToString <> pk) Then
                                    ...
                                    Else
                                        If UCase(row.Cells(i).Column.Key) = "USER_ID" Then
                                            ModifiedRow(i) = 0
                                        ElseIf UCase(row.Cells(i).Column.Key) = "TERMINAL_ID" Then
                                            ModifiedRow(i) = cboTerminals.SelectedValue
                                        ElseIf UCase(row.Cells(i).Column.Key) = "ACCESS_DATE" Then
        ...
        For Each dRow As DataRow In table.Rows
            If Not dRow.RowState = DataRowState.Unchanged Then
                If dRow.RowState = DataRowState.Deleted Then
                    objShuttle.Id = dRow.Item("Shuttle_Id", DataRowVersion.Original)
                    objShuttle.Delete(objShuttle.Id)
                Else
                    With objShuttle
                        ...
                        .UserID = objUser.Id
                        .ComputerName = "PC1"
                        .TerminalID = cboTerminals.SelectedValue
 
                    End With

                    If dRow.RowState = DataRowState.Added Then
                        objShuttle.Save(True)
                    ElseIf dRow.RowState = DataRowState.Modified Then
                        objShuttle.Id = "" & dRow.Item("Shuttle_Id")
                        objShuttle.Save(False)
                    End If
                End If
...
#Region " Functions And Procedure "
    ...
    Private Sub LoadCombo()
        ...
        dtFareDetail = objTerminal.GetAll()

        cboTerminals.DataSource = dtFareDetail
        cboTerminals.DataValueField = "Terminal_Id"
        cboTerminals.DataTextField = "Terminal_Name"

        cboTerminals.DataBind()
        'Dim getMaxVechile_Id As Integer = 0
        'getMaxVechile_Id = objShuttle.getMaxShuttle_Id()

        'dtFareDetail = objShuttle.GetAll()

        'Dim pk(0) As DataColumn
        'pk(0) = dtFareDetail.Columns("Shuttle_Id")
