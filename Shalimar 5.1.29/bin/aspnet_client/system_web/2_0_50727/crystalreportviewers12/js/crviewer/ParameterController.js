...
bobj.crv.params.ParameterController.prototype = {
    ...
    updateParameter: function(param) {
        ...
        if (param) {
            ...
            if (index != -1) {
                ...
                /*
                 *  Need to update isDirtyUnSelParam variable because the parameter getting updated might not be the currently selected parameter
                 *  for DCP cases
                 */
                
                if(paramUI !== this._selected) {
                    this.updateIsDirtyUnSelParams(paramUI.isDirty() || this.getIsDirtyUnSelParams());
                }
    ...
    updateIsDirtyUnSelParams : function(value) {
        this._isDirtyUnselParams = value ;
    },
    
    getIsDirtyUnSelParams : function() {
        return this._isDirtyUnselParams;
    },
    ...
    _getNumberParamValue: function(text,format) {
        ...
        var value = text.replace(gpRE,"").replace(format.decimalSeperator, ".");
        return value;
    },
    ...
    _onSelectValue: function(widget, valueIdx) {
        ...
        if (lastWidget && lastWidget.isDirty() && bobj.isNumber(lastValueIdx)) {
    ...
    _select: function(widget, valueIdx) {
        ...
        if (this._selected !== widget) {
            ...
            this.updateIsDirtyUnSelParams(false);
            var numParams = this._paramList.length;
            for (var i = 0; i < numParams; ++i) { 
                var paramWgt = this._panel.getParameter(i);
                if (paramWgt !== widget && this._panel.isDirty(i)) { 
                    this.updateIsDirtyUnSelParams(true);
                    break;
                }
            }
    ...
    _updateToolbar: function() {
        if (this._selected) {
            ...
            this._panel.setApplyButtonEnabled(this._selected.isDirty() || this.getIsDirtyUnSelParams());
        }
        else {
            this._panel.setApplyButtonEnabled(this.getIsDirtyUnSelParams());
    ...
    _checkAndSetValue: function(widget, valueIdx) {
         ...
         */
        if(!widget.isDirty() || !widget.canChangeOnPanel) {
            return;
        }
