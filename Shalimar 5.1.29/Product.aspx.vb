...
Partial Public Class Product
    ...
    Protected Sub Page_Load(ByVal sender As Object, ByVal e As System.EventArgs) Handles Me.Load
        ...
        objCity = New clsProduct(objConnection)
    ...
    Private Sub grdCity_InitializeLayout(ByVal sender As Object, ByVal e As Infragistics.WebUI.UltraWebGrid.LayoutEventArgs) Handles grdCity.InitializeLayout
        ...
        With e.Layout.Grid
            ...
            .Columns.FromKey("Product_Id").Hidden = True


            .Columns.FromKey("Product_Name").Header.Caption = "Product"
            .Columns.FromKey("Rate").Header.Caption = "Rate"

            .Columns.FromKey("Product_Name").Width = Unit.Percentage(50)
            .Columns.FromKey("Rate").Width = Unit.Percentage(40)
    ...
    Private Sub loadTable()
        ...
        Dim getMaxProduct_Id As Integer = 0

        getMaxProduct_Id = objCity.getMaxProduct_Id()
        dtCities = objCity.GetProduct()

        Dim pk(0) As DataColumn
        pk(0) = dtCities.Columns("Product_Id")
        pk(0).AutoIncrement = True
        dtCities.PrimaryKey = pk
        dtCities.AcceptChanges()

        If (dtCities.Rows.Count > 0) Then
            pk(0).AutoIncrementSeed = getMaxProduct_Id + 1
        Else
        ...
        For Each dRow As DataRow In table.Rows
            If Not dRow.RowState = DataRowState.Unchanged Then
                ...
                If dRow.RowState = DataRowState.Deleted Then
                    objCity.Id = dRow.Item("Product_Id", DataRowVersion.Original)
                    objCity.Delete()
                Else
                    With objCity
                        .Name = "" & dRow.Item("Product_Name")
                        .Rate = "" & dRow.Item("Rate")
                       
                    End With
                    If dRow.RowState = DataRowState.Added Then
                        objCity.Save(True)
                        'If ValidateCity("" & dRow.Item("Product_Name")) Then
                        '    lblError.Text = "City Name " & dRow.Item("Product_Name") & " or city abbr " & dRow.Item("Rate") & " is duplicate. Please resolve."
                        'Else
                    ...
                    ElseIf dRow.RowState = DataRowState.Modified Then
                        objCity.Id = "" & dRow.Item("Product_Id")
                        objCity.Save(False)
                    End If
                End If
    ...
    Private Function ValidateCity(ByVal Product_Name As String, ByVal City_Abb As String) As Boolean

        'ValidateCity
        Dim ds As DataSet

        ds = objValidate.ValidateCity(Product_Name, City_Abb)
