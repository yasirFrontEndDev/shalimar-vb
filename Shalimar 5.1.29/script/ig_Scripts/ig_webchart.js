...
function IGCrossHair(id, toggleOnClick)
{
	...
	this.Render=function(b)
	{
        ...
        var xOffset = 0, yOffset = 0, chartElement = document.getElementById(this.Parent.ID);
...
function IGScrollBar(id, width, height, scrollerLength, url, orientation, uniqueId) 
{
	...
	this.Render = function(where) 
	{
        ...
        if (IGB.IsFirefox && chartElement && tableElement && where_ref.offsetParent != chartElement && where_ref.offsetParent != chartElement.offsetParent)
        {
            var boxObj = document.getBoxObjectFor(chartElement);
            var offsetParentBoxObj = document.getBoxObjectFor(where_ref.offsetParent);
            if (boxObj && offsetParentBoxObj)
            {
                xOffset = boxObj.x - tableElement.offsetLeft;
                yOffset = boxObj.y - tableElement.offsetTop;
            }
        }
...
function IGUltraChart(id, imageUrl, uniqueId)
{
	...
	{   
        ...
        if (!positionFromBody)
		{
            ...
            if (IGB.IsFirefox && tooltip_ref_body.offsetParent != baseImageRef.offsetParent)
            {
        ...
        var offsetParent = tooltip_ref.offsetParent;
        var positionFromBody = offsetParent.tagName == "BODY" || offsetParent.tagName == "HTML"
        if (!positionFromBody)
		{
		    locx -= IGB.GetPageX(baseImageRef);
            locy -= IGB.GetPageY(baseImageRef);
            if (IGB.IsFirefox && tooltip_ref.offsetParent != baseImageRef.offsetParent)
            {
                var p = baseImageRef.offsetParent, i = 0, xOffset = 0, yOffset = 0;
		...
		if (this.EnableCrossHair)
		{
            ...
            if (IGB.IsFirefox && this.iGCrossHair.HairHorizontal.offsetParent != baseImageRef.offsetParent && positionFromBody)
            {
                var boxObj = document.getBoxObjectFor(baseImageRef);
                if (boxObj)
                {
                    xOffset -= boxObj.x;
                    yOffset -= boxObj.y;
		    ...
		    var hSC = IGB.GetHScrolledContainer(this.iGCrossHair.HairHorizontal);
		    var vSC = IGB.GetVScrolledContainer(this.iGCrossHair.HairHorizontal);
		    xOffset += hSC ? hSC.scrollLeft : 0;
		    yOffset += vSC ? vSC.scrollTop : 0;
			this.iGCrossHair.Visible = this.XhairBounds.Inside(locx + xOffset, locy + yOffset);
			this.iGCrossHair.Update(locx-3, locy-3);
