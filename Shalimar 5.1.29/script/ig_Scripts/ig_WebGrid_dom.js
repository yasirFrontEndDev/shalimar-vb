...
function(textValue,force)
{
	...
	if(typeof(value)!="undefined")
	{
		if(!textValue)
		{
			...
			if (this.Column.editorControl && this.Column.editorControl.toNum)
			{
	            value = this.Column.editorControl.toNum(value);
	        }
...
function(value,fireEvents)
{
	...
	if (this.Node) 
	{
		...
		var nodeValue=!v?this.Column.getNullText():v;
			...
			else
			{
				...
				else
				{
					...
					if(gs.get("StationaryMarginsOutlookGroupBy")=="True" && this.Band.Index==0 && this.Band.IsGrouped && i==0)
					{
						...
						var tbw=igtbl_parseInt(table.style.width)+c1w-this.getWidth();
						if(tbw>0)
							table.style.width=tbw.toString()+"px";
					}
				}
...
function(scrollLeft,force)
{
	...
	if(!divs.firstChild.style.width && this.Element.offsetWidth)
	{
		...
		if(!this._corr && this.IsXHTML && ig_csom.IsIE && this.Bands[0].Columns[0].Element)
		{
			...
			var columns = this.Bands[0].Columns;
			var column = columns[0];
			while(column && column.getHidden())
				column = columns[column.Index + 1];
			var colEl=column.Element;
			if(!colEl.offsetWidth)
			{
				var colEls=igtbl_getDocumentElement(column.Id)
				if(colEls.length && colEls.length>1)
					colEl=colEls[1];
			}
			corr=colEl.offsetWidth-colEl.clientWidth;
			corr+=igtbl_parseInt(colEl.currentStyle.paddingLeft);
			...
			var fCorr=0;
			while(i>0 && columns[columns.length-i].getFixed())
			{
				
				if(!columns[columns.length-i].getHidden())
				{
					j--;
					fCorr+=fcIncr;
				}
...
function()
{
    ...
    {
		// This logic is all necessary because of how the stationary margins deal with empty
		// grids. The grid actually has a hidden header row that belongs to main rows area 
		// and this header is displayed if there's no rows. Otherwise the header row that belongs
		// to the stationary margin area is displayed.
		// We need to move the filter row between the stationary header area and the non-stationary
		// one depending on whether there are rows visible or not. 
		// It's not necessary for footers because footers don't work this way.
		flr.parentNode.removeChild(flr);
