...
) 
{
   ...
   // alert("test");
	...
	else if(this.DataValueField)
	{
		...
		this.dataValue=this.getGrid().Bands[0].getColumnFromKey(this.DataValueField).getValueFromString(dataValueAsString);
	}
...
function igcmbo_imitateResize(comboId)
{
	...
	if(combo._imitationWidth!=combo.Element.parentNode.offsetWidth)
	{
...
function _igcmbo_setInputWidth(width)
{
	...
	if(ig_shared.IsNetscape6)
		window.setInterval("igcmbo_imitateResize('"+this.Id+"')", 500);
